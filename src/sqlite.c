...
const uint32_t USERNAME_OFFEST = ID_OFFEST + ID_SIZE;
const uint32_t EMAIL_OFFEST = USERNAME_OFFEST + USERNAME_SIZE;
...
void* row_slot(Table* table, uint32_t row_num) {
	...
	if (page == NULL) {
		//Allocate memory only when we tury to access page
		page = table->pages[page_num] = malloc(PAGE_SIZE);
	}
	uint32_t row_offest = row_num % ROWS_PER_PAGE;
	uint32_t byte_offest = row_offest * ROW_SIZE;
	return page + byte_offest;
}
